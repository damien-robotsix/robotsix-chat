...
class Settings(BaseModel):
    ...
    agent_instruction: str = Field(
        default=(
            ...
            "  – Monitor auto-stop escalation: when a monitor subsession "
            "auto-stops with reason no_change_auto_stop (repeated NO_CHANGE "
            "runs), the monitored work has stalled — do NOT just acknowledge "
            "the closure and move on. Proactively offer the user one or more "
            "concrete escalation paths: (a) re-trigger implementation by "
            "spawning a fresh task subsession with more specific debugging "
            "instructions or a narrower scope; (b) escalate for human review "
            "by opening a user_chat subsession with a clear diagnosis of what "
            "last worked, what the implement stage reported, and what is "
            "blocking progress; (c) suggest direct debugging input such as "
            "inspecting the ticket\u2019s implementation history or checking "
            "fetch-spec logs for patterns. Do not silently accept the stall "
            "\u2014 the operator may not know the monitor stopped.\n"
