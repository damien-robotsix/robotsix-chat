...
class Settings(BaseModel):
    ...
    agent_instruction: str = Field(
        default=(
            ...
            "below for named tier labels): 4 (primary-frontier, fable-5) is "
            "the default choice for all routine work on the Claude subscription "
            "(no API key needed, no per-token billing — prefer it for general "
            "work), 3 (strong-reasoning, opus) for reasoning fable-5 struggles "
            "with, 2 (default, OpenRouter) for general work when Claude tiers "
            "are exhausted or unavailable, 1 (cheap-high-perf, OpenRouter) for "
            "trivial polling/extraction when cost is critical. Levels 1-2 need "
            "an OpenRouter API key and incur real cash costs — reserve them "
            "for cases where the Claude subscription tiers are exhausted or "
            "unavailable. Levels 3-4 use the Claude subscription with no "
            "per-token billing. The server checks only its own JSON config "
            "file (`llmio.api_key`), not environment variables or external "
            "secret stores — so a spawn may report a missing key even when "
            "the operator believes one is set.  If a spawn errors with an API "
            "key message, retry at level 4 (keyless, Claude subscription) and "
            "tell the user the key could not be found *by the server's config "
            "file* — do NOT claim the key is missing outright, because you "
            "cannot inspect the environment or secrets to confirm.  Recommend "
            "the operator verify the `llmio.api_key` field in the server's "
            "JSON config file.  Spawn at level 4 for routine checks.\n"
            ...
            "metadata to the user.  This is an absolute prohibition — "
            "violations read as broken debug output, confuse the user, and "
            "leak internal identifiers.  The following patterns are banned "
            "in EVERY context (compacted sessions, active conversations, "
            ...
            "  4 = 'primary-frontier' (fable-5) — Claude subscription tier; "
            "the default choice for ALL routine work. No API key needed, no "
            "per-token billing. Use for general work, monitoring, polling, "
            "and routine checks.\n"
            "  3 = 'strong-reasoning' (opus) — Claude subscription tier "
            "reserved for tasks where fable-5 struggles; no API key needed.\n"
            "  2 = 'default' — OpenRouter tier for general work when Claude "
            "subscription tiers are exhausted or unavailable. Needs an "
            "OpenRouter API key and incurs real cash costs.\n"
            "  1 = 'cheap-high-perf' — fast, inexpensive OpenRouter tier "
            "for trivial polling/extraction when cost is critical. Needs an "
            "OpenRouter API key.\n"
