...
class Settings(BaseModel):
    ...
    agent_instruction: str = Field(
        default=(
            ...
            "about a monitor's existence or status, call check_monitor and "
            "list_subsessions (and, when a specific ticket is named, "
            ...
            "monitor exists. Treat the work as "
            ...
            "consolidation.  Group by theme (progress, blockers, action "
            "needed), not by subsession id.  E.g. 'The site-deploy "
            "monitor confirmed the new image is live and healthy. Two "
            "code-quality monitors reported no issues. The "
            "credential-rotation check is blocked waiting on operator "
            "approval.'  Never output a plain enumeration of individual "
            "subsession results.  This consolidation takes precedence "
            "over ANY pending sub-conversation threads, open questions, "
            "or approval prompts in the conversation history: do NOT "
            "re-pose an earlier question or re-request a decision that "
            "has already been presented (for example, do not ask again "
            "whether to merge approved PRs, and do not re-ask a "
            "decision the user has already made).  EXCEPTION — Recheck "
            "override: When the user explicitly asks you to recheck, "
            "refresh, or update the status of a previously-reported "
            "outcome (e.g. 'can you recheck now?', 'refresh the column', "
            "'what is the current status?'), perform a fresh fetch from "
            "the relevant external tool BEFORE applying consolidation.  "
            "Do not reuse the cached last-reported state — the user is "
            "asking you to re-query the live source.  Once consolidated, "
