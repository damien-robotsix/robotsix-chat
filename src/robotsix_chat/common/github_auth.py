...
async def _build_github_app_auth_headers(dr: DirectRepoSettings, label: str) -> str | None:
    """Mint a GitHub App installation token and return the raw token string."""
    if dr.github_app_id and dr.github_app_private_key.get_secret_value() and dr.github_app_installation_id:
        from robotsix_github_auth import mint_installation_token
        try:
            result = await asyncio.to_thread(mint_installation_token, ...)
