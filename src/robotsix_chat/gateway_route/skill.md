...
route. central-deploy derives its edge routing table automatically from the component registry:
every registered, routable component with `id == <slug>` is published at
`<slug>.<gateway_base_domain>`, and the registry entry's container name/port is the upstream. There
is no per-service routing rule — a route "exists" exactly when the slug is present in the registry.
...
- Pinpoint a missing route before reporting to the user — the tool returns the current vhost →
  upstream mapping and a direct "route present / missing" conclusion.
...
- `expected_route` — `"<service_slug>.<gateway_base_domain>"` (the vhost central-deploy would
  publish)
- `route_present` — `true` when the slug appears in the registry (so the edge publishes it)
- `matching_mappings` — registry entries whose `component_id` equals the slug, with `vhost` and
  `upstream`
...
- `route_present: false` — the service is missing from the registry. The edge derives no vhost for
  it, so no gateway route exists. This is the "missing route" case: report it to the user and route
  the remediation to onboarding/infra rather than continuing to probe the public URL.
