...
mark_ticket_done(ticket_id: str, justification: str = "") -> str
...
Validation is best-effort: when the repo list is unreachable (network error, timeout), the check is
skipped and filing proceeds without validation — the board API's own 404 is the fallback guard.
