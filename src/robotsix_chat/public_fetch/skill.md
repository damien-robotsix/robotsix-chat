...
- **Auth refusal** — HTTP 401/403 responses on non-fleet-auth hosts are reported with a clear
  error message. Fleet-auth hosts have credentials injected server-side and should not see
  401/403 for auth reasons; if they do, the credentials may need updating.
