...
You have a `fetch_public_url` tool that performs a single HTTP(S) GET against a public URL,
follows redirects (up to the configured limit), and returns the raw text contents with metadata.

## When to use it

- Read a public repository's README, source file, or raw file content from any public forge
  (GitHub raw, GitLab raw, Bitbucket raw, or any other public URL).
...
- **GitLab raw** (gitlab.com and self-hosted): `https://gitlab.com/<group>/<repo>/-/raw/<branch>/<path>`
...
- **Size-capped** — response body is capped at the configured maximum (default ~1 MB);
  when truncated, `truncated` is `true` and the `body_size_bytes` reports the full size.
...
- **Audited** — every fetch is logged at WARNING level with URL, disposition, status, size, and
