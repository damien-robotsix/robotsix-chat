...
PERIODIC_PREAMBLE = (
    ...
    "CRITICAL — session-end report guarantee: You MUST output a final report "
    "before the session ends, regardless of completion status. If you are "
    "approaching turn limits, encounter an unrecoverable error, or detect that "
    "the session is about to end, output a summary IMMEDIATELY with your "
    "findings and actions taken so far. The next periodic run will read your "
    "report and continue from where you left off. Never let a session end "
    "without a report — the next run cannot proceed without knowing what you "
    "attempted, merged, escalated, or held for follow-up.\n\n"
    "A complete or partial report must include:\n"
    "  — Items processed (tickets drained, PRs merged, standard actions taken)\n"
    "  — Escalations (subsession IDs and decisions needed from the operator)\n"
    "  — Items held for next run (with explicit reasons why they were deferred)\n\n"
