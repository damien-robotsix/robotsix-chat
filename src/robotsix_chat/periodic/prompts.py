...
PERIODIC_PREAMBLE = (
    ...
    "  - Wrap every tool call and API request in error handling. Do NOT rely "
    "on tools succeeding silently — anticipate and catch failures. When a "
    "tool fails (GitHub API 500, board API timeout, component unreachable), "
    "log the failure, skip that item, and continue with the next one if "
    "possible.\n"
    "  - On ANY unexpected error — tool failure, API error, timeout, "
    "exception — immediately emit a PARTIAL REPORT capturing:\n"
    "      * What you completed successfully (e.g. 'Checked 5 PRs')\n"
    "      * What you were doing when the error occurred (e.g. 'checking PR "
    "      #42 from robotsix-chat')\n"
    "      * The error details (e.g. 'GitHub API returned 500: Internal Server "
    "      Error')\n"
    "      * What items are being deferred (e.g. 'PRs #43-50 not checked due "
    "      to API error')\n"
    "    Example: 'Done: 5 PRs checked and merged. Error: GitHub API returned "
    "    500 during PR enumeration. Held for next run: 12 remaining PRs, "
    "    cannot enumerate due to GitHub API failure.'\n"
    "  - CRITICAL — error-handling rule: an error is NOT permission to skip "
    "the report. No matter what goes wrong — tool crash, API failure, timeout, "
    "out-of-memory, context exhaustion — your LAST action before the session "
    "ends MUST be to emit a PARTIAL REPORT. If you have context left, emit a "
    "complete report with error details. If context is almost gone, emit what "
    "you can (header and summary counts first, before details). A partial "
    "report in the face of an error is infinitely more valuable to the next "
    "run than silence and a cryptic failure.\n"
    "  - If the error happens while you are already emitting the report "
    "(mid-PARTIAL REPORT output), STOP immediately. What you have output so "
    "far is valid and will be captured — do not attempt to continue or append "
    "once an error is in progress.\n\n"
