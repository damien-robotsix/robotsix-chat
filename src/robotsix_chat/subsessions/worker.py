...
def _build_periodic_input(
    ...
    parts.append(
        ...
        "repo's primary publish/deploy workflow.  If the workflow run "
        "failed or is still in progress with failures on prior runs, do "
        "NOT call complete_subsession with a success summary — the fix "
        "did not actually resolve the pipeline failure.  Instead:\n"
        "  - If the workflow failed: call complete_subsession with a "
        "summary that INCLUDES the workflow failure details (run id, "
        "failure reason, and log excerpt if available), and set the "
        "close reason to 'escalated' or 'failed'.  The summary must make "
        "clear that the ticket was closed but the CI pipeline is still "
        "failing — this breaks the redraft loop.\n"
