...
_REACT_PROMPT_TEMPLATE = (
    ...
    "This rule takes precedence over ANY pending sub-conversation "
    ...
    "ask again whether to merge approved PRs).  "
    "DUPLICATE-DECISION DETECTION — Before reacting to a monitor outcome, "
    "scan the conversation ABOVE this system notice for mentions of the "
    "same ticket or subject.  If the conversation already shows that "
    "you surfaced a recommendation, decision, or approval request for "
    "this ticket to the user — and the monitor outcome merely restates "
    "the same 'needs your decision' or approval status without a NEW "
    "delta — treat the outcome as NO_CHANGE.  Either OMIT it from your "
    "response entirely, or collapse it into a single non-actionable "
    "note such as 'The monitor for ticket X restated the same decision "
    "already presented above — no new information.'  Never re-present "
    "a decision the conversation already shows was surfaced.  "
    "EXCEPTION — Recheck "
    ...
    "block IDs (hex strings like 'a3f2', 'block a3f2'), raw internal "
    "identifiers (ticket IDs, run numbers, PR numbers, commit hashes), "
    "state machine transitions, spawn counters, internal timeout values, "
    "stack traces, raw API response fragments, "
    ...
    "IMPORTANT: before labeling a ticket FULFILLED or 'closed without "
    "implementation', cross-reference the monitor's outcome against any "
    "evidence of active work — PR merge events, state transitions through "
    "pipeline stages (ready → code_review → documenting → deliverable → "
    "implement_complete → waiting_auto_merge / human_mr_approval → done → "
    "closed — the real mill state names), a pr_url on the ticket, or "
    "merge/approval events in the ticket history.  If evidence of completed "
    "work exists, say 'Tracking complete — the ticket's work was delivered' "
    "— NOT 'closed without implementation — re-file needed'.  Only "
    "recommend re-filing when the history genuinely shows the ticket was "
    "dropped (e.g. draft → closed with no intervening work activity and "
    "no pr_url).  A closed ticket with a PR is delivered, not dropped.\n"
    ...
    "details that the subsession agent included in its summary — ticket "
    "IDs, run numbers, PR numbers, commit hashes, block IDs, event "
    "numbers, state machine transitions, spawn counters, internal "
...
_BATCH_REACT_PROMPT_TEMPLATE = (
    ...
    "{outcomes_list}\n"
    "CONSOLIDATION RULE (MANDATORY — ALWAYS apply, this overrides every "
    ...
    "individual outcome.  Never output the raw outcome list as-is, even "
    "as bullet points.  Omit trivial NO_CHANGE runs entirely — mention "
