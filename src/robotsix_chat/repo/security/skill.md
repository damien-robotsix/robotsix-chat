...
Fetch the plain-text log for a GitHub Actions job. The GitHub API returns a 302 redirect to a signed
URL; this endpoint follows it server-side and returns the log content directly as a 200 response so
the agent can inspect deploy pipeline output (e.g., lftp commands) to diagnose deployment failures.
