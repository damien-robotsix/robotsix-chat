...
Fetches the PR to verify it exists and is open, resolves its head branch, and pushes the given
files as a new commit.  This is the CI-fix iteration path — no new branch is created and no
BLOCKED-state or implement-cycle gate is enforced.

**Configuration gate:** This tool is only available when `allow_push_to_existing_pr` is `true`
in the config (default `false`).
