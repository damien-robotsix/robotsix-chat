...
def build_autonomous_instruction(settings: Settings) -> str:
    ...
    return (
        ...
        "\n"
        "The same rule applies after a service restart.  The startup "
        "resume hook re-spawns most monitors automatically, but monitors "
        "that were already terminal or explicitly stopped are NOT "
        "re-spawned.  After receiving a restart notice, check which "
        "monitors were listed as interrupted or NOT resumed.  For every "
        "interrupted monitor whose tracked ticket is still open, spawn a "
        "replacement monitor immediately as your first action.\n"
        "\n"
