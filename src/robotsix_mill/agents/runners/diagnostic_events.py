...
Shadow override of the installed ``robotsix_mill.agents.runners.diagnostic_events``
— adds WARNING-level log messages to every early-return path in
``emit_diagnostic_event`` so that skipped events are immediately visible in logs.
"""
...
def emit_diagnostic_event(
    ...
    new event is skipped and ``False`` is returned with a WARNING log
    explaining why.  Otherwise the event is appended and ``True`` is
    returned.

    Fail-safe: any I/O error is logged at WARNING and ``False`` is
    returned (the caller must not break on a failed event write).
    """
    try:
        ...
        if _event_exists(path, ticket_id, normalized_key):
            log.warning(
                ...
                "ticket=%s category=%s board_id=%s key=%s",
                ticket_id,
                category,
                board_id,
                normalized_key,
        ...
        log.warning(
            "diagnostic_events: failed to emit event — "
            "ticket=%s category=%s board_id=%s key=%s",
            ticket_id,
            category,
            board_id,
