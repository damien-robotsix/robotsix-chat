...
def review_input_hash(
    ...
    diff, the branch-tip HEAD SHA, a fingerprint of the reviewer
    itself, AND the modification time of ``implement.md`` (a proxy for
    "has the implement stage produced a new pass since the last
    review?").

    Including *head_sha* ensures that after a rebase or force-push (new
    HEAD SHA) the cache misses even when the diff text is unchanged,
    forcing a fresh review against the current branch tip.

    Including the reviewer fingerprint does the same when mill's own
    review prompt or a repo's conventions change — see
    :func:`reviewer_fingerprint` for why that matters.

    Including ``implement.md``'s mtime ensures the cache misses after
    every implement pass — even when the diff and spec are identical
    (e.g. a release-please repo where the reviewer keeps flagging a
    missing changelog fragment that the implement agent correctly
    declines to add).  Without a monotonic signal, a REQUEST_CHANGES
    verdict cached under the pre-implement hash replays indefinitely,
    burning the implement/review ceiling with no progress.  Each
    implement pass rewrites ``implement.md``, so the mtime advances
    and the hash changes — the cache misses and a fresh review runs.
    """
    h = hashlib.sha256()
    ...
    # Fold in implement.md's mtime as a monotonic cycle counter: every
    # implement pass rewrites this file, so the hash changes even when
    # the diff, spec, HEAD SHA, and reviewer fingerprint are unchanged.
    try:
        implement_md = ws.artifacts_dir / "implement.md"
        if implement_md.exists():
            mtime_ns = str(implement_md.stat().st_mtime_ns)
            h.update(mtime_ns.encode("utf-8", errors="replace"))
    except OSError:
        log.debug("Failed to stat implement.md for review input hash", exc_info=True)
