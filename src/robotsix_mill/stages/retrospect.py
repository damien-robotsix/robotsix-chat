...
class RetrospectStage(Stage):
    ...
    def run(self, ticket: Ticket, ctx: StageContext) -> Outcome:
        ...
        if pr_entries:
            ...
            # Single-repo verification: confirm the PR recorded in
            # merge.md is still present and merged on the forge before
            # allowing the ticket to close.  merge.md is written by the
            # merge stage when it transitions the ticket to DONE; its
            # absence means the ticket reached DONE through a
            # no-change / manual path, which is fine.  Forge errors are
            # non-blocking — transient retry is the merge stage's job.
            merge_md = ws.artifacts_dir / "merge.md"
            if merge_md.exists():
                try:
                    merge_text = merge_md.read_text(encoding="utf-8")
                except OSError:
                    merge_text = ""
                # merge.md format: "merged: <url>" or "auto-merged: <url>"
                # or "closed-empty: <url>".  Extract the URL.
                m = re.search(r"https?://[^\s]+", merge_text)
                if m:
                    pr_url = m.group(0)
                    ...
                    if pr is None or not pr.get("merged"):
                        return Outcome(
                            State.BLOCKED,
                            "retrospect refusing to close — "
                            "PR recorded in merge.md is no longer "
