...
- **Tracing**: cognee traffic uses its **own** Langfuse project — named by
  `memory.langfuse_project` (default `robotsix-chat-cognee`) and resolved against the
  canonical top-level `langfuse.projects` block — never the main `robotsix-chat` project's
  credentials (component standard: one Langfuse project per LLM-generating function).
