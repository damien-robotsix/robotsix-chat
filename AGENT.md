...
**Rule:** When adding or editing ordered lists in `docs/`, use `1.` for every item (CommonMark
renders them sequentially regardless). Do not use sequential numbering (`2.`, `3.`, …) — the
`mdformat --number` pre-commit hook flattens them back to `1.`, creating a wasted CI auto-fix cycle.
This applies to all Markdown files under `docs/`.
