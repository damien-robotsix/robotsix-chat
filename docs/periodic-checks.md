...
**Stale-monitor suppression.** If the monitored ticket is already in a terminal state — that is,
its `last_known_state` is `"closed"` or `"done"` — the auto-stop/auto-pause notification is silently
