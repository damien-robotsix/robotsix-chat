...
| `subsessions.stale_reclaim_seconds`                    | `number`        | `0.0`                      | When the global pool is full but the spawning session is under its per-session limit, PAUSED subsessions from other sessions that have been idle for longer than this many seconds are reclaimed (closed) to free a slot.  Set to `0` to disable.                                                                                                                                                                                                                                                                                                                                                  |
