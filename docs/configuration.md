...
| `chat_model_level`              | `integer` or `null` | `null`                                                | Optional override of `llmio_model_level` for the main interactive chat agent. When `null` (default), the chat agent uses `llmio_model_level`. Set to a specific level (e.g. `3` for the frontier tier) to route chat turns to a different level while other consumers (subsessions, autonomous, summary) still use `llmio_model_level` or their own overrides.                                                                                                                                                                                                                                            |
...
| `model_level`                  | `integer` | `null`       | llmio capability level for this autonomous session (1 cheap/frequent, 2 workhorse, 3 frontier). When `null`, the session uses the global `llmio_model_level`. Set per-preset to run cheap monitors on level 1 while keeping hard work on levels 2-3. |
