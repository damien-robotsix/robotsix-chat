...
| --------------------------- | ------------------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `llmio_model_level`         | `integer`           | `4`                                                   | LLM capability level: `1` (cheapest) … `5` (best); `2` is keyless Claude haiku.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
1. **Live-proof:** start a chat session and ask the agent to perform one of the listed actions. The
...
1. **Post-deploy follow-up:** after the next deployment cycle, verify that the `low_risk_actions`
...
| `memory.autonomous_enabled`                    | `boolean`         | `false`                          | When `true`, the autonomous agent gets full memory. Default `false` for the same cost reason as `subsession_enabled`. Independent toggle so the two background classes can be gated separately.                                                                                                                                                                                                                                                                                     |
...
| `subsessions.default_model_level`                       | `integer`       | `4`                        | Default model level for spawned subsessions. Keyless tier (Claude SDK) for guaranteed availability — long-lived background trackers survive provider outages.                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `subsessions.delegated_read_model_level`                | `integer`       | `2`                        | Default model level for children spawned (without an explicit `model_level`) by a costly-tier parent subsession (level `>=3`). Costly-tier agents are instructed to fan bulk reading/extraction out to cheap children; this makes that fan-out land on a cheap tier by default while the parent can still override `model_level` when a subtask genuinely needs reasoning.                                                                                                                                                                                                                          |
| `subsessions.monitor_max_model_level`                   | `integer`       | `2`                        | Maximum model level for periodic and wait_for_event monitor subsessions. Routine monitors (ticket polling, periodic checks) are capped at this level to prevent them from burning expensive keyless Claude subscription tiers. Levels 1-2 use OpenRouter (cheap real $); levels 3-4 use keyless Claude SDK (subscription cap). Set to `4` to remove the cap.                                                                                                                                                                                                                                        |
...
| `autonomous.continue_interval_seconds`              | `number`  | `45.0`                        | Retained for config compatibility. No longer used — autonomous runs receive a single prompt, so there is no auto-continue pacing loop.                                                                                                                 |
| `autonomous.max_idle_auto_turns`                    | `integer` | `5`                           | Retained for config compatibility. No longer used — there is no auto-continue loop, so there is no idle turn cap.                                                                                                                                      |
...
| `autonomous.queue_tolerance_runs_before_escalation` | `integer` | `3`                           | Number of consecutive `NO_CHANGE` monitor cycles to accept as queue wait before escalating a possible stall on a serial board. During this window the agent checks whether earlier-queued tickets are actively being worked.                           |
| `autonomous.ready_staleness_minutes`                | `integer` | `10`                          | Number of minutes a ticket can remain in the `ready` state before it is considered stale. The `list_stale_ready_tickets` tool surfaces tickets that have been sitting in `ready` longer than this threshold.                                           |
| `autonomous.priority_ready_staleness_minutes`       | `integer` | `60`                          | Number of minutes a priority-flagged ticket can remain in the `ready` state before it is considered stale. Priority tickets get a longer grace period because they are often waiting in a serial implementation queue rather than being truly stalled. |
...
| `model_level`                  | `integer` | `null`       | llmio capability level for this autonomous session (1 cheapest … 4 frontier). When `null`, the session uses the global `llmio_model_level`. Set per-preset to run cheap monitors on level 1-2 while keeping frontier work on level 3-4. |
| `max_runs`                     | `integer` | `0`          | Maximum number of times this preset may fire. After the preset has completed `max_runs` runs it is automatically disabled. `0` means unlimited.                                                                                         |
...
| `autonomy.auto_approve_well_scoped_low_risk`        | `boolean` | `false` | When `true`, the agent may auto-approve self-authored `human_issue_approval` tickets that are well-scoped, low-risk, and carry explicit acceptance criteria — even when the target repo is not in `auto_approve_repo_allowlist`. The agent must verify: the spec has concrete, verifiable acceptance criteria; the change has no external dependencies; the scope is bounded and non-controversial; and the change is non-destructive and reversible. The agent MUST log the auto-approval decision with a summary of why the ticket qualified. |
...
| `autonomy.auto_escalate_secret_scan_alerts`         | `boolean` | `true`  | When `true`, the agent auto-escalates tickets blocked on a secret-scan alert (e.g. TruffleHog verified secrets) by filing a credential-rotation follow-up ticket and notifying the operator with a summary and recommended next steps.                                                                                                                                                                                                                                                                                                          |
| `autonomy.operator_review_escalation_hours`         | `integer` | `48`    | Number of hours a ticket can remain in awaiting-operator (`ASK_USER` / held-for-review) state before the agent re-prompts the operator with a summary and asks whether to continue waiting or take an alternative action.                                                                                                                                                                                                                                                                                                                       |
...
"autonomy": {
  ...
  "auto_approve_well_scoped_low_risk": true,
  "suppress_no_change_monitors": true,
  "auto_self_restart": true,
  "auto_escalate_secret_scan_alerts": true,
  "operator_review_escalation_hours": 48
}
```
