...
Keys are component IDs matching the central-deploy roster (the ``id`` field in ``GET /chat/components``). The roster entry's ``auth.type`` selects which credential fields are used — only the fields matching the declared auth scheme are consulted when authenticating.
