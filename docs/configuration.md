...
| `llmio.subagent_model` | `LLMIO_SUBAGENT_MODEL` | `"sonnet"` | Bare Claude model name override for background sub-agents spawned via `delegate_task`. Only applied when the foreground level uses the keyless claudeSDK provider (level 3). `sonnet` (default) is cheaper than Opus; set `haiku` for cheaper delegation, or empty/null to disable the downgrade. |
| `llmio.check_loop_model` | `LLMIO_CHECK_LOOP_MODEL` | `"haiku"` | Bare Claude model name override for check-loop workers (recurring monitoring / status-check ticks). Only applied at level 3 (claudeSDK). `haiku` (default) is the cheapest subscription tier — ideal for binary "is it done yet?" polling. Set `sonnet` for more nuanced monitoring, or empty/null to disable the downgrade. |
