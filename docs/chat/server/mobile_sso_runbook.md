...
This runbook covers operational procedures for the mobile SSO authentication endpoints
(`GET /auth/login`, `GET /auth/callback`, `POST /chat/auth/mobile-token`).

## Overview

The mobile SSO flow allows the mobile app to authenticate users via tinyauth and obtain short-lived
bearer tokens for API access. The flow is:

1. **Login initiation**: Mobile app opens `GET /auth/login?redirect_to=<deep-link>` in an external
   browser.
1. **Tinyauth authentication**: User authenticates at the tinyauth edge proxy.
1. **Callback**: Tinyauth redirects to `GET /auth/callback`, which redirects to the mobile app's
   deep-link.
1. **Token exchange**: Mobile app calls `POST /chat/auth/mobile-token` to exchange the tinyauth
   identity header for a short-lived bearer token.
...
Set `mobile_auth.enabled` to `false` in the configuration file and restart the service:
...
If you suspect token compromise, rotate the `token_secret` to invalidate all existing tokens
immediately:
...
1. Update `mobile_auth.token_secret` in the config file.
...
Alert rules are defined in `deploy/alerts/mobile_sso.yml`. Deploy this file to your Prometheus
server's rules directory.
...
1. If attack suspected: consider rate-limiting or temporarily disabling.
1. If mass expiry: check when tokens were issued and consider extending TTL.
1. If misconfiguration: verify tinyauth is setting the identity header.
...
1. If legitimate domain: add to `allowed_redirect_domains`.
1. If attack: investigate source and consider blocking.
...
1. If clock skew: verify NTP synchronization.
1. If secret mismatch: ensure all instances use the same `token_secret`.
...
1. If unintentional: check configuration and restart.

## Audit Logging

All auth-sensitive operations are logged with structured fields for audit trail purposes. Logs are
emitted via structlog in JSON format when `structured_logging` is enabled.
...
The current implementation does not use persistent subject tokens. The tinyauth identity header is
used directly for token exchange. This means:

- **No server-side token storage**: Tokens are HMAC-signed and stateless.
- **No individual revocation**: Cannot revoke a single user's tokens without rotating the shared
  secret.
- **Revocation by secret rotation**: Rotating `token_secret` invalidates ALL existing tokens
  immediately.
...
1. **Investigate**: Check audit logs for unusual token issuance patterns.
1. **Notify**: Inform affected users they need to re-authenticate.

### Suspected attack

1. **Rate limiting**: Consider adding rate limiting to auth endpoints.
1. **IP blocking**: Block suspicious IPs at the edge proxy.
1. **Temporary disable**: Set `enabled: false` if attack is severe.

### Edge proxy misconfiguration

1. **Verify headers**: Ensure tinyauth sets `X-Forwarded-User` correctly.
1. **Check bypass**: Ensure `/chat/auth/mobile-token` is accessible without browser SSO (the app
   calls it programmatically).
1. **Test flow**: Verify the complete login flow on a test device.
