...
"Periodic session speed complaints" and refers to scheduled periodic sessions. (2) New "Mill
approval gate" block: the assistant is the approver for tickets at `human_issue_approval` — review
the spec against robotsix-standards and act with `transition_ticket` (sound → ready; thin → draft
with a note naming the gap; duplicate/obsolete → draft then closed), never spawn wait-for-human
monitors or report "waiting for approval".

**Why:** Autonomous sessions were replaced by periodic sessions — ordinary chat sessions started on
a schedule with one initial prompt; the dedicated runner, protocol supplement, and state machine
were removed. Separately, the operator removed the human from the mill approval loop: the chat
master reviews and approves/redirects `human_issue_approval` tickets itself.

**SHA256:** `0e5c8aa03e0b28c804b4c2cff39a4abd614f8d0b6f70f6f3e8b6923dc1b5333a`
...
## Autonomous Prompt Changelog (retired)

The autonomous appendix (`build_autonomous_instruction()` in
`src/robotsix_chat/autonomous/prompts.py`, versions AUTONOMOUS v1–v53) was retired in
main-prompt v153: autonomous sessions were replaced by periodic sessions, which are plain
chat sessions started on a schedule with an initial prompt and no appended protocol.  The
full AUTONOMOUS entry history and its governance policy are preserved in git history
(`git log -- docs/system_prompt_changelog.md` before the v153 entry).
