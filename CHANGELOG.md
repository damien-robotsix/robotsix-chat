...
- **Breaking:** Removed static `feedback.repo_ids` config key and `FEEDBACK_TARGET_REPOS` env override. Allowed feedback target repos are now resolved dynamically at run time from the deploy server's chat-component roster intersected with the mill board's repo registry — no chat-side config needed. Falls back to `["robotsix-chat"]` when deploy is unreachable.
