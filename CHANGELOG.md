...
- Added API-level output capping (`agent.max_output_tokens`, default 1200; `agent.output_stop_sequences`) passed through to the LLM as `model_settings` so the assistant's completion tokens are hard-capped regardless of prompt compliance. Complements the v14 system-prompt Efficiency tightening with an enforcement mechanism that catches the ~2,211-token status-read bloat observed in trace 0119d46d.
- Extracted the three inner tool closures from `build_check_loop_tools` in `delegation.py` to module-level async functions (`_start_check_loop_tool`, `_stop_check_loop_tool`, `_list_check_loops_tool`) that take captured state as explicit keyword arguments, reducing nesting and making each tool independently testable.
- Scoped the "new tickets default to robotsix-mill" system-prompt claim to `consult_mill` specifically, replacing a false universal statement ("regardless of source") with accurate board-manager-default wording.  `SYSTEM_PROMPT_VERSION` bumped to 12.
- Tightened system-prompt Efficiency rules: three-sentence limit reframed as a hard quota-backed cap; added targeted brevity rules for status reads, board queries, and check-loop tick replies; strengthened tool-loading-narration prohibition. `SYSTEM_PROMPT_VERSION` bumped to 14.
