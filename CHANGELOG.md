...
- Default `server_host` to `0.0.0.0` (bind all interfaces) instead of `127.0.0.1` — inside a container the loopback default causes silent gateway 502. Add a persistent named config volume in `deploy/docker-compose.yml` so the operator-managed config survives image updates.
