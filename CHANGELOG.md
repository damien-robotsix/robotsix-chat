...
- Convert `mock_app` and `http_client` in `tests/conftest.py` from `@asynccontextmanager` helpers into `@pytest.fixture` async generators.  New fixtures: `agent` (session scope), `app` (session scope), `async_client` (function scope), and `mock_app` (function scope, customisable via indirect parametrization).  Migrate all test functions in `tests/chat/test_server.py` and `tests/chat/test_events.py` to use fixture injection instead of `async with mock_app(...) as f:`.
