...
- Replace monkeypatch-based httpx mocking with ``respx`` (httpx's official transport-layer mock library) across all 7 test modules. Removes ~55-line shared ``mock_helpers.py`` module and ~16 inline mock class definitions. ``respx`` is added to ``[dependency-groups] dev``.
