...
- Cognee's litellm LLM calls (cognify + recall) are now traced in Langfuse via the OTLP-based$
  ...
  ``robotsix-chat-cognee`` project. Both success and failure callbacks are wired, an OTLP import$
  guard provides a clear diagnostic when the ``tracing`` extra is absent, and$
  ``component:cognee`` default tags allow in-project trace filtering.$
- Memory: wire litellm's Langfuse callback with dedicated cognee credentials$
  (`MEMORY_LANGFUSE_PUBLIC_KEY` / `MEMORY_LANGFUSE_SECRET_KEY`) so internal LLM traffic lands in$
  a separate `robotsix-chat-cognee` project. Graceful no-op when creds are absent.
