...
- **Autonomous sessions: complete lifecycle integration.** The autonomous session system now has full server-side enforcement across the critical integration points: (1) the chat endpoint rejects messages in ``awaiting_approval`` state with 409, (2) ``completion_marker`` and ``approval_marker`` are inspected against every agent reply in the coalescer, triggering real state transitions, (3) the ``AutonomousRunner`` is wired through ``create_app`` and ``run_server_from_config`` with ``resume_autonomous_sessions`` on startup, (4) approval triggers an auto-continue turn so the agent executes without waiting, (5) ``max_auto_turns`` is enforced per execution phase, and (6) private ``_store._owners``/``_store._sessions`` access is replaced by public ``owner_for_session``/``iter_sessions`` methods. Added 17 integration tests covering the full lifecycle.
- **Autonomous sessions** — native, first-class autonomous chat sessions with a built-in lifecycle loop: subject selection → plan drafting → mandatory approval gate → execution → auto-close + respawn. No subsession nesting required. Autonomous sessions are visually distinguished in the UI with an `[AUTONOMOUS]` title prefix and a `--autonomous-accent` CSS variable for color/badge customization. Gated behind `autonomous.enabled` (default `false`).
