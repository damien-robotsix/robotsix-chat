...
- Dockerfile: migrate from `/opt/venv` copy pattern to canonical `uv export --frozen` + `uv pip install --system` pattern, installing directly into the runtime image's system Python.  Removes the builder-stage virtualenv indirection; build-only tooling (git, uv binary) is pruned from the final image.
