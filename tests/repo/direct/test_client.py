...
# ============================================================================
# DirectRepoClient — create_repo release-automation bootstrap
# ============================================================================
#
# changelog-driven-releases.md §4 requires every repo to wire the shared
# auto-release workflow. A fleet sweep found nine repos that never did and so
# had never cut a release tag, which is why this happens at creation time
# rather than being left to a later step.


@pytest.mark.asyncio
async def test_create_repo_non_fleet_name_skips_bootstrap(
    respx_mock: respx.MockRouter,
) -> None:
    """A repo without the fleet prefix is left exactly as before."""
    from tests.repo.direct.conftest import _prepopulate_installation_token, _settings
    ...
    respx_mock.post("https://api.github.com/orgs/myorg/repos").mock(
        return_value=httpx.Response(201, text=json.dumps({"html_url": "u"}))
    )

    result = await client.create_repo(org_name="myorg", repo_name="some-tool")
    assert "created successfully" in result
    assert "Release automation" not in result
    # Creation is the only API call made.
    assert len(respx_mock.calls) == 1


@pytest.mark.asyncio
async def test_create_repo_fleet_name_wires_release_automation(
    ...
    async def _fake_bootstrap(self: object, repo_full_name: str) -> list[str]:
        seen["repo"] = repo_full_name
        return ["var ok", "secret ok", "workflow ok"]

    from robotsix_chat.repo.direct.actions_client import ActionsClient

    monkeypatch.setattr(
        ActionsClient, "bootstrap_release_automation", _fake_bootstrap, raising=True
    )

    result = await client.create_repo(org_name="myorg", repo_name="robotsix-newthing")
    assert seen["repo"] == "myorg/robotsix-newthing"
    assert "Release automation" in result
    assert "workflow ok" in result
    # The repo still reports as created.
    assert "created successfully" in result


@pytest.mark.asyncio
async def test_bootstrap_failure_does_not_fail_creation(
    monkeypatch: pytest.MonkeyPatch, respx_mock: respx.MockRouter
) -> None:
    """A half-provisioned repo is finishable by hand; a failed creation is not."""
    from tests.repo.direct.conftest import _prepopulate_installation_token, _settings
    ...
    async def _boom(self: object, repo_full_name: str) -> list[str]:
        raise RuntimeError("app not installed")

    from robotsix_chat.repo.direct.actions_client import ActionsClient

    monkeypatch.setattr(
        ActionsClient, "bootstrap_release_automation", _boom, raising=True
    )
...
@pytest.mark.asyncio
async def test_bootstrap_can_be_disabled(respx_mock: respx.MockRouter) -> None:
    from tests.repo.direct.conftest import _prepopulate_installation_token, _settings
    ...
    result = await client.create_repo(
        org_name="myorg", repo_name="robotsix-newthing", bootstrap_release=False
    )
    assert "Release automation" not in result
    assert len(respx_mock.calls) == 1
