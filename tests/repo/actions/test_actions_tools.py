...
# ---------------------------------------------------------------------------
# fetch_workflow_job_log — scope check
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_fetch_job_log_refuses_out_of_scope_repo(
    respx_mock: respx.MockRouter,
) -> None:
    """Repo not in installation scope → refused message."""
    dr = _direct_repo_settings()
    ...
    tools = build_github_actions_tools(_actions_settings(), dr)
    fetch_job_log = tools[4]

    result = await fetch_job_log("other-repo", 42, "build")
    assert "not installed" in result.lower()
    assert "other-repo" in result


# ---------------------------------------------------------------------------
# fetch_workflow_job_log — no jobs in run
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_fetch_job_log_no_jobs(
    respx_mock: respx.MockRouter,
) -> None:
    """Workflow run with no jobs → descriptive message."""
    dr = _direct_repo_settings()
    ...
    result = await fetch_job_log("test-repo", 99, "build")
    assert "no jobs found" in result.lower()
    assert "99" in result


# ---------------------------------------------------------------------------
# fetch_workflow_job_log — job name not found
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_fetch_job_log_name_not_found(
    respx_mock: respx.MockRouter,
) -> None:
    """Job name not in run → error with available jobs listed."""
    dr = _direct_repo_settings()
    ...
    result = await fetch_job_log("test-repo", 42, "build")
    assert "no job named 'build'" in result.lower()
    assert "'lint'" in result
    assert "'test (3.14)'" in result


# ---------------------------------------------------------------------------
# fetch_workflow_job_log — happy path
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_fetch_job_log_success(
    respx_mock: respx.MockRouter,
) -> None:
    """Happy path — fetches and returns job log text."""
    dr = _direct_repo_settings()
    ...
    # Mock the job log endpoint — the client follows the redirect
    # to a signed URL, so we mock the initial API URL with follow_redirects.
    respx_mock.get(
        url__startswith=(
            f"{dr.github_api_base_url}/repos/damien-robotsix/test-repo"
            "/actions/jobs/101/logs"
        )
    ).respond(
    ...
    result = await fetch_job_log("test-repo", 42, "test (3.14)")
    assert "FAILED tests/test_foo.py" in result
    assert "AssertionError" in result


# ---------------------------------------------------------------------------
# fetch_workflow_job_log — truncation of large logs
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_fetch_job_log_truncates_large_log(
    respx_mock: respx.MockRouter,
) -> None:
    """Logs longer than 200 KB are truncated to the trailing portion."""
    dr = _direct_repo_settings()
    ...
    # Build a log larger than 200 KB (204800 bytes).
    prefix = "A" * 250_000
    suffix = "Z" * 30_000
    large_log = prefix + suffix
    ...
    # Should contain the truncation header and the trailing suffix.
    assert "truncated" in result.lower()
    assert "Z" * 100 in result
    # Result length must be bounded (header + last 200 KB of log).
    assert len(result) < 210_000
    # Verify the result starts with the truncation header (not raw log).
    assert result.startswith("# Job log:")
