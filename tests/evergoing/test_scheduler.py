...
def _store_with_session(turns: int) -> tuple[ConversationStore, str]:
    store = ConversationStore()
    meta = store.create_session(OPERATOR_OWNER)
...
def test_gate_skips_at_keep_recent_runs_without_watermark_advance() -> None:
    ...
    store, sid = _store_with_session(turns=5)
    agent = _CountingAgent("a summary")
...
def test_summary_failure_leaves_session_untouched() -> None:
    store, sid = _store_with_session(turns=7)
...
def test_compact_session_cover_until_index() -> None:
    ...
    store, sid = _store_with_session(turns=6)
...
def test_create_app_wires_summary_scheduler_without_creating_sessions() -> None:
    """No session is created on boot; the compaction scheduler is still wired.

    The evergoing session (created on boot) was removed on 2026-09-15; the
    scheduler exists whenever a summary agent does.
    """
    store = ConversationStore()
    agent = _CountingAgent("a summary")
    ...
    app = create_app(
        ...
        evergoing_settings=EvergoingSettings(trim_interval_seconds=60.0),
    )

    assert store.all_session_ids() == []
    assert isinstance(app.state.evergoing_scheduler, EvergoingSummaryScheduler)
    assert app.state.evergoing_scheduler.interval_seconds == 60.0


def test_legacy_enabled_key_is_dropped_on_load() -> None:
    """Deployed configs still pin ``evergoing.enabled`` — it must not crash the boot."""
    settings = EvergoingSettings.model_validate(
        {"enabled": True, "keep_recent_runs": 3}
    )
    assert settings.keep_recent_runs == 3
