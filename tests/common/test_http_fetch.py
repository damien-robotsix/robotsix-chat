...
Covers the four SSRF / fleet-auth / allowlist / URL-scheme helpers directly,
...
class TestBuildFleetAuthHeader:
    """Unit tests for :func:`_build_fleet_auth_header`."""

    def test_none_fleet_auth_returns_no_header(self):
        """None config yields no header and empty auth_hosts."""
        header, auth_hosts = _build_fleet_auth_header(None)
        assert header is None
        assert auth_hosts == set()

    def test_empty_username_returns_no_header(self):
        """Empty username is treated as missing credentials."""
        settings = FleetAuthSettings(
            basic_auth_username="",
            basic_auth_password="secret",  # pragma: allowlist secret
            auth_hosts=["h1"],
        )
        header, auth_hosts = _build_fleet_auth_header(settings)
        assert header is None
        assert auth_hosts == set()

    def test_empty_password_returns_no_header(self):
        """Empty password is treated as missing credentials."""
        settings = FleetAuthSettings(
            basic_auth_username="op",
            basic_auth_password="",  # pragma: allowlist secret
            auth_hosts=["h1"],
        )
        header, auth_hosts = _build_fleet_auth_header(settings)
        assert header is None
    ...
    def test_valid_credentials_yields_basic_auth_header(self):
        """Valid creds produce a Basic-Auth base64 header and auth_hosts set."""
        settings = FleetAuthSettings(
            basic_auth_username="user",
            basic_auth_password="pass",  # pragma: allowlist secret
            auth_hosts=["deploy.robotsix.net", "internal.org"],
        )
        header, auth_hosts = _build_fleet_auth_header(settings)
        assert header == "Basic dXNlcjpwYXNz"
        assert auth_hosts == {"deploy.robotsix.net", "internal.org"}

    def test_credentials_with_special_characters(self):
        """Colon in password is base64-encoded correctly."""
        settings = FleetAuthSettings(
            basic_auth_username="u",
            basic_auth_password="p:w",  # pragma: allowlist secret
            auth_hosts=[],
        )
        header, auth_hosts = _build_fleet_auth_header(settings)
        # "u:p:w" → base64
        assert header == "Basic dTpwOnc="
        assert auth_hosts == set()
