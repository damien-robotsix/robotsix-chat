...
def _register_subsession(registry: SubsessionRegistry, *, owner: str, kind: SubsessionKind=SubsessionKind.TASK, title: str='job', **overrides: object) -> SubsessionInfo:
    ...
    return registry.create(kind=kind, owner_session_id=owner, parent_id=None, depth=1, title=title, prompt='do the thing', model_level=3, **overrides)
...
@pytest.mark.asyncio
async def test_ui_injects_history_load_path(mock_app: AppFixture) -> None:
...
@pytest.mark.asyncio
async def test_create_agent_from_settings_uses_load_when_none(monkeypatch: pytest.MonkeyPatch) -> None:
    """``create_agent_from_settings`` resolves config when *settings* is None."""
    from robotsix_chat.config import Settings
    with patch('robotsix_chat.config.settings.load_config', return_value=Settings(llmio_model_level=1, llmio_api_key=SecretStr('sk-env-test'))):
...
@pytest.mark.asyncio
async def test_run_server_from_config_creates_agent_from_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from robotsix_chat.config import Settings
    with patch('robotsix_chat.config.settings.load_config', return_value=Settings(llmio_model_level=3, server_host='127.0.0.1', server_port=8080)), patch('robotsix_chat.chat.server.run_server') as mock_run_server:
        run_server_from_config()
        ...
        assert call_args[1] == {'host': '127.0.0.1', 'port': 8080, 'idle_timeout_minutes': 30, 'max_images_per_message': 8, 'max_image_bytes': 5242880, 'allowed_image_media_types': ['image/png', 'image/jpeg', 'image/gif', 'image/webp'], 'cors_allow_origins': [], 'correlation_id_header': 'X-Request-ID'}

@pytest.mark.asyncio
async def test_run_server_from_config_passes_explicit_agent(monkeypatch: pytest.MonkeyPatch) -> None:
    """Forward an explicit agent to ``run_server``.

    ``run_server_from_config(agent)`` passes *agent* through without
    creating a new one.
    """
    with patch('robotsix_chat.config.settings.load_config', return_value=Settings()):
...
@pytest.mark.asyncio
async def test_ui_renders_event_stream_wiring(mock_app: AppFixture) -> None:
...
async def test_subsessions_list_returns_owner_tree(mock_app: AppFixture) -> None:
    ...
    for snapshot in subsessions:
        ...
        for key in ('subsession_id', 'kind', 'owner_session_id', 'parent_id', 'depth', 'title', 'prompt', 'model_level', 'status', 'created_at', 'last_activity_at', 'interval_seconds', 'next_run_at', 'include_previous_result', 'runs', 'max_runs', 'last_result', 'summary', 'close_reason', 'error'):
...
async def test_subsessions_transcript_endpoint(mock_app: AppFixture) -> None:
    ...
    data = response.json()
    assert data['subsession_id'] == info.id
    assert [(e['role'], e['text']) for e in data['transcript']] == [('user', 'a question'), ('assistant', 'an answer')]
...
async def test_subsessions_close_cancels_worker_and_delivers_summary(mock_app: AppFixture) -> None:
    ...
    sub_id = spawn_subsession(env=env, kind=SubsessionKind.TASK, owner_session_id='sess-live', parent_id=None, depth=1, title='long job', prompt='work forever', model_level=3)
...
def test_main_agent_gets_per_request_subsession_tools_factory() -> None:
    ...
    assert names == ['spawn_subsession', 'message_subsession', 'close_subsession', 'list_subsessions']
...
def test_subsession_agent_gets_static_tools_with_complete() -> None:
    ...
    agent = create_agent_from_settings(settings=settings, subsession_env=env, subsession_ctx=ctx, subsession_close_state=CloseState())
...
class TestExportLangfuseEnv:
    ...
    def test_exports_base_url_and_host(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        sk = 'sk-lf-test'
        settings = Settings(langfuse=LangfuseSettings(public_key=SecretStr('pk-lf-test'), secret_key=SecretStr(sk), host='https://langfuse.example.com'))
