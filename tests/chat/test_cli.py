...
from robotsix_chat.chat.server.cli import (
    _configure_logging,
    _maybe_import_config_from_central_deploy,
...
class TestMaybeImportConfigFromCentralDeploy:
    """Unit tests for ``_maybe_import_config_from_central_deploy``."""

    def test_skips_when_lifecycle_disabled(self) -> None:
        """No-op when lifecycle.enabled is False."""
        settings = Settings(
        ...
        with patch("robotsix_chat.chat.server.cli.logger") as mock_logger:
            _maybe_import_config_from_central_deploy(settings)

        # No log message emitted — the function returned immediately.
        mock_logger.info.assert_not_called()
        mock_logger.warning.assert_not_called()

    def test_skips_when_config_import_disabled(self) -> None:
        """No-op when lifecycle.config_import_enabled is False."""
        settings = Settings(
            lifecycle=LifecycleSettings(
                enabled=True,
                config_import_enabled=False,
                base_url="http://central-deploy:8100",
                service_name="test-svc",
            )
        )
    ...
    def test_skips_when_config_file_exists_and_nonempty(self) -> None:
        """No-op when the resolved config file already exists with content."""
        import json
        ...
        mock_path.read_text.return_value = json.dumps({"llmio_model_level": 3})

        with (
            patch(
                "robotsix_config.resolve_config_path",
                return_value=mock_path,
            ),
            patch("robotsix_chat.chat.server.cli.logger") as mock_logger,
        ):
            _maybe_import_config_from_central_deploy(settings)

        # Should log debug about skipping.
        mock_logger.debug.assert_called_once()
        assert "exists" in mock_logger.debug.call_args[0][0]

    def test_happy_path_imports_and_persists(self) -> None:
        """Happy path: missing config → import, validate, write, bootstrap."""
        import json
        from pathlib import Path
        ...
        imported = {"llmio_model_level": 4, "log_level": "DEBUG"}

        mock_path = MagicMock(spec=Path)
        mock_path.exists.return_value = False

        mock_client = MagicMock()
        mock_client.import_service_config = MagicMock(return_value=json.dumps(imported))
        ...
        with (
            ...
            patch(
                "robotsix_chat.lifecycle.client.LifecycleClient",
                return_value=mock_client,
            ),
            patch(
                "robotsix_chat.chat.server.cli.asyncio.run",
                return_value=json.dumps(imported),
            ),
            patch("robotsix_chat.chat.server.cli.logger") as mock_logger,
        ):
            _maybe_import_config_from_central_deploy(settings)

        # Verify the config was written (inline tmp.write_text + tmp.replace).
        mock_path.with_suffix.assert_called_once()
        mock_path.with_suffix.return_value.write_text.assert_called_once()
        mock_path.with_suffix.return_value.replace.assert_called_once_with(mock_path)
        # Should log success.
        mock_logger.info.assert_called()
        assert "imported" in mock_logger.info.call_args[0][0].lower()

    def test_import_failure_logs_warning(self) -> None:
        """When central-deploy returns an error, a warning is logged."""
        from pathlib import Path
        ...
        with (
            ...
            patch(
                "robotsix_chat.chat.server.cli.asyncio.run",
                return_value="Lifecycle: connection refused",
            ),
            patch(
                "robotsix_chat.lifecycle.client.LifecycleClient",
            ),
